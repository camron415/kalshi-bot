...
pub async fn run_ws(
    ...
    let request = Request::builder()
        ...
        .body(())?;
    let (ws_stream, _) = connect_async(request).await.context("Failed to connect to websocket")?;
    info!("[KALSHI] Connected");
    let (mut write, mut read): (futures_util::stream::SplitSink<_, _>, futures_util::stream::SplitStream<_, _>) = ws_stream.split();
        ...
        match msg {
            Ok(Message::Text(text)) => {
                match serde_json::from_str::<KalshiWsMessage>(&text) {
                    Ok(kalshi_msg) => {
                        // Handle orderbook updates here for predictive logic
                        // (Implementation to be added in main loop)
                    }
                    Err(e) => {
...
impl KalshiApiClient {
    pub async fn get_active_markets(&self) -> anyhow::Result<Vec<KalshiMarket>> {
        // TODO: Replace with real API call
        Ok(vec![KalshiMarket {
            ticker: "CPI2026".to_string(),
            question: "Will CPI rise in 2026?".to_string(),
            price: 0.45,
            volume: Some(1200),
        }])
    }
